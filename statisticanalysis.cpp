    ...
    ui(new Ui::StatisticAnalysis)
{
    ...
    connect(ui->pushButton_query, SIGNAL(clicked()), this, SLOT(queryStatisticAnalysis()));
...
void StatisticAnalysis::queryStatisticAnalysis()
{
    ...
    }else if(ui->comboBox->currentText() == QString::fromLocal8Bit("周报"))
    {
        ...
        beginDateTime = QString("%1-%2-%3 00:00:00").arg(queryDate.toString("yyyy")).arg(queryDate.addDays(dayOfWeek - 1).month()).arg(queryDate.addDays(dayOfWeek - 1).day());
        endDateTime = QString("%1-%2-%3 23:59:59").arg(queryDate.toString("yyyy")).arg(queryDate.addDays(7 - dayOfWeek).month()).arg(queryDate.addDays(7 - dayOfWeek).day());
